...
ENV SOURCE_URL=http://www.squid-cache.org/Versions/v6/squid-${SQUID_VERSION}.tar.gz
...
RUN rm -r /etc/apt/sources.list.d/* \
 ...
 && mkdir /build \
 && curl -o /build/squid-source.tar.gz ${SOURCE_URL} \
 && cd /build \
 ...
 && make install \
 && cp -rf icons/* /usr/share/squid/icons/ \
...
RUN cp -rf /etc/squid/mime.conf.default /etc/squid/mime.conf
